...
fn update_boids(boids: &mut Vec<Boid>, grid: HashMap<(u32, u32), Vec<usize>>) {
    ...
    let mut new_boid_states: Vec<(f32, f32, f32, f32, f32)> = Vec::with_capacity(boids.len());

    for (boid_idx, boid) in boids.iter().enumerate() {
        ...
        new_boid_states.push((next_x, next_y, next_xv, next_yv, speed));
    }
    // apply the changes
